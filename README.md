RavensMart V1.3 — PhunEconomy

A configurable Project Zomboid Build 42 economy built around PhunMart 2. Also known as PhunEconomy.

Current release: RavensMart_V1.3. Its six JSON files match the supplied RavensMart_V1.3.zip exactly. The Ravensmart, ravensmart_V1.1, and RavensMart_V1.2 folders are historical releases; use only the V1.3 files for the current economy.

V1.3 changes vehicle trade-ins to 12 bound tokens, full gold ingots to 72, full silver ingots to 24, and standard premium vehicles to 150. Gold and silver coins are removed from the custom collector list. The Street Sweeper retains its 24-token price override.
...
PhunKeys — provides the Transmutation Box and vehicle seller key

PhunCure — provides PhunCure.Cure, stocked by CSV Pharmacy
...
Vehicle trade-ins use PhunKeys.VehicleKeySeller at Prawn Stars. V1.3 does not override HardWear.
...
Gold Ingot (Base.GoldBar): 72 bound tokens per ingot

Silver Ingot (Base.SilverBar): 24 bound tokens per ingot

Gems: Amethyst and Sapphire $10, Ruby $11, Emerald $12, Diamond $13

Leather Harness Panties payout $1

Transmuted vehicle seller key (PhunKeys.VehicleKeySeller): 12 bound tokens
...
Generator variants are blacklisted from the configured normal tool and electronics pools.
...
Wrent-A-Wreck also sells the PhunKeys Transmutation Box (PhunKeys.TransmutationBox) for $1.
...
Use PhunKeys to transmute a vehicle and trade the resulting PhunKeys.VehicleKeySeller at Prawn Stars for 12 bound tokens. This configuration assigns the same payout to every seller key; it does not appraise the original vehicle's condition or rarity.

At the 150-token standard premium price:

12 trade-ins earn 144 tokens, leaving 6 tokens to earn elsewhere.

13 trade-ins earn 156 tokens, enough to buy one standard premium vehicle with 6 tokens left.

The 24-token Street Sweeper costs the equivalent of 2 trade-ins.

Gold and silver ingots pay at a 3:1 ratio: 72 tokens for gold and 24 for silver. Collectibles provide another source of tokens.

The JSON does not impose per-vehicle trade-in exclusions. Which vehicles can be transmuted is determined by PhunKeys.
...
The current group contains 140 entries, including:
...
Install and enable all content mods referenced by the configuration.

Back up the existing Zomboid/Lua/PhunMart_*.json files.

Copy all six PhunMart_*.json files from RavensMart_V1.3/ directly into Zomboid/Lua, replacing the prior versions. Do not mix files from historical release folders.

Confirm the required Workshop IDs and Mod IDs are present in the server INI.

Restart the server so PhunMart recompiles the definitions.

Force-restock modified machines through the PhunMart admin tools.

Confirm that referenced item and vehicle script names resolve during startup.

Test purchases and trade-ins before promoting configuration changes to a production server.

The V1.3 package contains only the six economy override files. It does not supply PhunMart.json or PhunServer2Cron.json; retain your existing server settings for those files.
...
The six V1.3 files have been checked for valid JSON, duplicate object keys, and internal custom group, pool, price, and reward references. They depend on PhunMart base definitions, including pool_prawnstars_core, self_1, currency_base, token_base, token_24, and token_collector_t1. These base definitions and installed content IDs must resolve in the server's installed mod versions. Repository synchronization is not an in-game purchase or trade-in test.
